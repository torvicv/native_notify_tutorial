...
import { z } from "zod";
import AddBonos from "../components/addBonos";
...
export default function Create() {
  ...
  const onSubmit = async (data) => {
    ...
    const response = await fetch('/api/admin/bonos', {
      method: 'POST',
      ...
      <form className="max-w-md mx-auto" onSubmit={handleSubmit(onSubmit)}>
        <div className="grid md:grid-cols-1 md:gap-6">
          ...
          <div className="relative z-0 w-full mb-5 group">
            ...
            {errors.precio && <p className="text-red-600">{errors.precio.message}</p>}
          </div>
          <div className="relative z-0 w-full mb-5 group">
            <textarea
        ...
        <AddBonos setValue={setValue} addDetalle={detallesSeleccionados} />
        {errors.detalles?.message && <p className="text-red-500">{errors.detalles.message}</p>}

        {errors.detalles?.map((error, index) => (
